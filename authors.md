# Authors and Citation

## Authors

- **Johan Larsson**. Author, maintainer.
  [](https://orcid.org/0000-0002-4029-5945)

- **Jonas Wallin**. Author. [](https://orcid.org/0000-0003-0381-6593)

- **Malgorzata Bogdan**. Author.
  [](https://orcid.org/0000-0002-0657-4342)

- **Ewout van den Berg**. Author.

- **Chiara Sabatti**. Author.

- **Emmanuel Candes**. Author.

- **Evan Patterson**. Author.

- **Weijie Su**. Author.

- **Jakub Kała**. Author.

- **Krystyna Grzesiak**. Author.

- **Mathurin Massias**. Author.

- **Quentin Klopfenstein**. Author.

- **Michal Burdukiewicz**. Author.
  [](https://orcid.org/0000-0001-8926-582X)

- **Jerome Friedman**. Contributor.  
  code adapted from 'glmnet'

- **Trevor Hastie**. Contributor.  
  code adapted from 'glmnet'

- **Rob Tibshirani**. Contributor.  
  code adapted from 'glmnet'

- **Balasubramanian Narasimhan**. Contributor.  
  code adapted from 'glmnet'

- **Noah Simon**. Contributor.  
  code adapted from 'glmnet'

- **Junyang Qian**. Contributor.  
  code adapted from 'glmnet'

## Citation

Source:
[`inst/CITATION`](https://github.com/jolars/SLOPE/blob/v2.1.1/inst/CITATION)

Larsson J, Bogdan M, Grzesiak K, Massias M, Wallin J (2025). “Efficient
Solvers for SLOPE in R, Python, Julia, and C++.”
[doi:10.48550/arXiv.2511.02430](https://doi.org/10.48550/arXiv.2511.02430).
2511.02430.

    @Misc{,
      title = {Efficient Solvers for {{SLOPE}} in {{R}}, {{Python}}, {{Julia}}, and {{C}}++},
      author = {Johan Larsson and Małgorzata Bogdan and Krystyna Grzesiak and Mathurin Massias and Jonas Wallin},
      number = {arXiv:2511.02430},
      year = {2025},
      month = {nov},
      eprint = {2511.02430},
      doi = {10.48550/arXiv.2511.02430},
    }

Bogdan M, van den Berg E, Sabatti C, Su W, Candès E (2015). “SLOPE –
Adaptive Variable Selection via Convex Optimization.” *The Annals of
Applied Statistics*, **9**(3), 1103–1140. ISSN 1932-6157.
[doi:10.1214/15-AOAS842](https://doi.org/10.1214/15-AOAS842).

    @Article{,
      title = {{{SLOPE}} -- Adaptive Variable Selection via Convex Optimization},
      author = {Małgorzata Bogdan and Ewout {van den Berg} and Chiara Sabatti and Weijie Su and Emmanuel J. Candès},
      journal = {The Annals of Applied Statistics},
      volume = {9},
      number = {3},
      pages = {1103--1140},
      year = {2015},
      doi = {10.1214/15-AOAS842},
      issn = {1932-6157},
    }

Larsson J, Klopfenstein Q, Massias M, Wallin J (2023). “Coordinate
Descent for SLOPE.” In *Proceedings of the 26th International Conference
on Artificial Intelligence and Statistics*, volume 206 series
Proceedings of Machine Learning Research, 4802–4821.

    @InProceedings{,
      title = {Coordinate Descent for {{SLOPE}}},
      author = {Johan Larsson and Quentin Klopfenstein and Mathurin Massias and Jonas Wallin},
      journal = {Proceedings of Machine Learning Research},
      volume = {206},
      pages = {4802--4821},
      publisher = {PMLR},
      series = {Proceedings of Machine Learning Research},
      booktitle = {Proceedings of the 26th International Conference on Artificial Intelligence and Statistics},
      address = {Valencia, Spain},
      year = {2023},
      month = {apr},
    }
